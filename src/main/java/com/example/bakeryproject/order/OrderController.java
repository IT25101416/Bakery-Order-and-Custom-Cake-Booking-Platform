...
public class OrderController {
    ...
    private final OnlineOrderProcessor orderProcessor = new OnlineOrderProcessor();

    public OrderController(OrderService orderService, CakeService cakeService, SavouryService savouryService) {
    ...
    @GetMapping("/place")
    public String showPlaceOrderPage(@RequestParam(required = false) String cakeError,
                                     Model model,
                                     HttpSession session) {
        ...
        if ("CUSTOMER".equals(session.getAttribute("role")) && session.getAttribute("customerName") != null) {
    ...
    public String placeOrder(@ModelAttribute Order order, HttpSession session) {
        ...
        Cake selectedCake = cakeService.findCakeByName(order.getCakeName());
        Savoury selectedSavoury = savouryService.findSavouryByName(order.getCakeName());

        if ((selectedCake == null && selectedSavoury == null) || order.getQuantity() <= 0) {
        ...
        double total = orderProcessor.calculateTotal(unitPrice, order.getQuantity());
    ...
    public String showCustomerOrderStatus(@RequestParam(required = false) String orderId,
        ...
        List<Order> myOrders = orderService.getOrdersByCustomerName(customerName);
        ...
        if (orderId != null && !orderId.trim().isEmpty()) {
            ...
            } else if (searchedOrder.getCustomerName() == null || !searchedOrder.getCustomerName().equalsIgnoreCase(customerName)) {
    ...
    public String showAdminStatusPage(@PathVariable String id, Model model, HttpSession session) {
        if (!"ADMIN".equals(session.getAttribute("role"))) {
            return "redirect:/customer-home";
        }
    ...
    @PostMapping("/status")
    public String updateStatus(@RequestParam String orderId, @RequestParam String status, HttpSession session) {
        if (!"ADMIN".equals(session.getAttribute("role"))) {
            return "redirect:/customer-home";
        }
