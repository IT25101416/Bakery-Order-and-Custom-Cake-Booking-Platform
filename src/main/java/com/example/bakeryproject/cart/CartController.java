...
public class CartController {
    ...
    public CartController(CakeService cakeService, SavouryService
            savouryService) {
    ...
    public String addToCart(@PathVariable String id, HttpSession session) {
        ...
        if (cake != null) {
            ...
            if (!found) {
                cart.add(new CartItem(cake.getCakeId(), cake.getCakeName(),
                        cake.getCategory(), cake.getPrice(), 1));
            }
    ...
    public String addSavouryToCart(@PathVariable String id, HttpSession
            session) {
        if (session.getAttribute("role") == null) {
            return "redirect:/login";
        }
        ...
        if (savoury != null) {
            ...
            if (!found) {
                cart.add(new CartItem(savoury.getSavouryId(),
                        savoury.getSavouryName(), savoury.getCategory(), savoury.getPrice(), 1));
            }
    ...
    public String removeFromCart(@PathVariable String id, HttpSession
            session) {
        List<CartItem> cart = getCart(session);
